...
def f_custom_astar(node, goal_state):
    ...
    '''
    EXPLANATION:
        1. From the cost function we can infer that changing the orientation (turning the bot)
           results in a lot of cost and many paths.
        2. Manhattan distance doesn't account for this, as it is sum of 
           So even though it is optimal the heuristic makes the algorithm to explore
           all the valid paths with little changes in directions. Which causes the
           algorithm to explore too many paths and expand too many nodes.
           For example:
               Say we have a n*n square map, and 4 corner vertices be a, b, c, d.
               Let initial state be at a, and goal at c.
               Say we have 3 paths,
               1. bottom-left to top-left (0,0 -> 0,b) & top-left to 0,b -> b,c
